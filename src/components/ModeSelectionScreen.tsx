...
export function ModeSelectionScreen({ onSelect }: ModeSelectionScreenProps) {
  return (
    <div
      className="fixed inset-0 z-50 flex flex-col items-center justify-center neo-grid px-4 py-8 overflow-y-auto select-none"
      style={{
        '--background': '#FFF8E7',
        '--surface': '#FFFFFF',
        '--text': '#000000',
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-3xl flex flex-col items-center text-center my-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Radio className="w-7 h-7 text-black p-1 bg-[var(--accent)] border-2 border-black rounded shadow-[2px_2px_0px_rgba(0,0,0,1)]" aria-hidden="true" />
          <span className="font-space text-2xl md:text-3xl font-black tracking-tight text-[var(--text)] uppercase">
            WAVESCAPE
          </span>
        </div>

        {/* Main Title & Prompt */}
        <h1 className="font-space text-3xl sm:text-4xl md:text-5xl font-black uppercase text-black tracking-tight mb-3">
          What would you like to tune into?
        </h1>
        <p className="font-mono text-sm sm:text-base text-[var(--muted-text)] max-w-lg mb-8 md:mb-10">
          Select your media frequency below to enter the global atmospheric broadcast receiver.
        </p>

        {/* Cards Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 w-full max-w-2xl px-2">
          {/* TV Selection Card */}
          <button
            type="button"
            onClick={() => onSelect('tv')}
          ...
          >
            ...
            {/* Icon Badge */}
            <div className="w-20 h-20 mb-5 rounded-2xl border-3 border-black bg-sky-300 shadow-[4px_4px_0px_rgba(0,0,0,1)] flex items-center justify-center group-hover:scale-105 transition-transform duration-200">
              <Tv className="w-10 h-10 text-black" />
            </div>

            {/* Title */}
            <h2 className="font-space text-2xl font-black uppercase text-black mb-2 tracking-tight">
              Live TV
            </h2>

            {/* Description */}
            <p className="font-mono text-xs sm:text-sm text-[var(--muted-text)] leading-relaxed mb-8">
              Real-time international IPTV television streams with categorized regional channels.
            </p>

            {/* Enter Button CTA */}
            <div className="w-full mt-auto py-2.5 px-4 font-space font-black text-sm uppercase tracking-wider border-2 border-black rounded-lg bg-sky-300 text-black group-hover:bg-black group-hover:text-white shadow-[2px_2px_0px_rgba(0,0,0,1)] transition-colors duration-150">
              Enter TV →
            </div>
          </button>
          ...
          >
            ...
            {/* Icon Badge */}
            <div className="w-20 h-20 mb-5 rounded-2xl border-3 border-black bg-[var(--accent)] shadow-[4px_4px_0px_rgba(0,0,0,1)] flex items-center justify-center group-hover:scale-105 transition-transform duration-200">
            ...
            {/* Enter Button CTA */}
            <div className="w-full mt-auto py-2.5 px-4 font-space font-black text-sm uppercase tracking-wider border-2 border-black rounded-lg bg-[var(--accent)] text-black group-hover:bg-black group-hover:text-white shadow-[2px_2px_0px_rgba(0,0,0,1)] transition-colors duration-150">
              Enter Radio →
            </div>
          </button>
        </div>
        ...
        {/* Footer Note */}
        <p className="mt-8 font-mono text-xs text-[var(--muted-text)]">
